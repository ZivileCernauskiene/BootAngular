...
export class MemoryComponent implements OnInit {
  ...
  usedClases(x: number) {
      if (this.maxUsedClasses <= 2 && x < 0) {
          alert('Please select more elements, even a child can solve more');
          this.maxUsedClasses = 2;
          return;
      }

      if (this.maxUsedClasses >= this.availableClasses.length && x > 0) {
          alert('Sorry I do not believe you can solve this much. Try contacting me if you want more classes to be added');
